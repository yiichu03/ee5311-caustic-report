...
- All three cases show clear convergence. The loss drops by about 72.49% for `GROUP23`, 72.45% for `NUS`, and 62.89% for `EE5311`, and the GIFs show a consistent coarse-to-fine optimization trend.
- 三个 case 都表现出清晰的收敛趋势。`GROUP23` 的 loss 下降约 `72.49%`，`NUS` 下降约 `72.45%`，`EE5311` 下降约 `62.89%`，而 GIF 也都显示出从粗到细的统一优化过程。
...
- `GROUP23` is the strongest reconstruction in this run, with the lowest mean difference error of `0.1198`.
- `GROUP23` 是这次实验里重建效果最好的 case，它的平均差值误差最低，为 `0.1198`。
...
- `NUS` is the middle case, with a mean difference error of `0.1320`.
- `NUS` 处于中间水平，平均差值误差为 `0.1320`。
...
- `EE5311` is the most challenging case here, with the highest mean difference error of `0.1655`.
- `EE5311` 是这里最有挑战性的 case，平均差值误差最高，为 `0.1655`。
